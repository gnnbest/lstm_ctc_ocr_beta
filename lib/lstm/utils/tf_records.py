...
'''
    ...
    nw = tf.cast(((nh * width) / height),tf.int32)

    image = tf.image.resize_images(image,size=(nh, nw),method=tf.image.ResizeMethod.BILINEAR)

    image = tf.cond((nw > w), lambda :tf.image.resize_images(image, size=(nh, w), method=tf.image.ResizeMethod.BILINEAR),
            lambda :tf.image.pad_to_bounding_box(image, 0, 0, nh, w) )

    image = tf.transpose(image, perm=[1, 0, 2])
    image = tf.cast(tf.reshape(image, [w, cfg.NUM_FEATURES]), dtype=tf.float32)/255

    time_step = tf.constant((int)(w/cfg.POOL_SCALE), tf.int32)
'''
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while not coord.should_stop():
                # get one batch
                image, label, label_len, time_step = sess.run([image,label,label_len,time_step])
...
if __name__=='__main__':
    ...
    wrtie_test(img_path='../../utils/data/train',tfrecords_filename='../../utils/data/train.tfrecords')
   # read_test(tfrecords_fiename='/media/nas/ocr/lstm_ctc_ocr_beta/lib/utils/data/val.tfrecords')
